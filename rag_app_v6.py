...
# --- [NEW] 구조화된 답변 생성 (근거 인용 + 확신도 + 요약→근거→결론) ---
def generate_answer(question, context_chunks):
    # 청크에 [출처 N] 번호 부여 → LLM이 인용 가능
    numbered_context = "\n\n".join(
...
1. 반드시 아래 3단계 구조로 답변하세요:
   ...
   **📖 근거**
   각 주장마다 반드시 [출처 N] 형태로 참고한 청크 번호를 인용.
   예: "계약 기간은 1년입니다. [출처 2]"
   여러 출처를 함께 인용해도 됩니다: [출처 1][출처 3]

   **✅ 결론**
   최종 판단을 작성하고, 마지막 줄에 반드시 확신도를 명시.
   형식: 확신도: 높음 / 보통 / 낮음 — (이유 한 줄)

2. 확신도 기준:
   - 높음: 컨텍스트에 명확하고 직접적인 근거가 있음
   - 보통: 관련 내용은 있으나 직접적인 답변이 아님, 추론이 필요함
   - 낮음: 관련 내용이 매우 부족하거나 간접적임

3. 컨텍스트에 전혀 없는 내용은 근거 섹션에 "관련 내용을 찾을 수 없습니다."라고 쓰고 확신도를 '낮음'으로 표시하세요.
4. 절대로 컨텍스트에 없는 내용을 지어내지 마세요.
5. 항상 한국어로 답변하세요."""
...
with st.sidebar:
    ...
    chunking_mode = st.radio(
        ...
        help="의미 기반은 문장 임베딩 유사도로 자연스러운 주제 경계를 찾습니다 (처리 시간 더 소요)"
    )
            ...
            with st.spinner("문서 처리 중..."):
                for f in uploaded_files:
                    ...
                    if chunking_mode == "의미 기반(Semantic) + Overlap":
                        chunks = chunk_text_semantic(text, chunk_size=chunk_size, overlap=overlap)
                    else:
    ...
    st.markdown("---")
    st.caption("Powered by OpenAI + FAISS | v6: Structured Answer + Citation")


# --- 메인 화면 ---
st.title("💬 문서 기반 챗봇 v6")
st.caption("근거 인용 · 확신도 표시 · 요약→근거→결론 구조")
            ...
            with st.spinner("답변 생성 중..."):
                ...
                if use_query_rewrite:
                    ...
                    candidate_chunks = [
                        ...
                        for i in indices[0] if i < len(st.session_state.chunks)
                    ]
                ...
                # 출처 문서 표시 — 번호를 답변의 [출처 N]과 일치시킴
                with st.expander("📄 출처 문서 보기 ([출처 N] 대응)"):
