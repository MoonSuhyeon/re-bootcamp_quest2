...
LOG_FILE        = os.path.join(_BASE, "rag_eval_logs_v12.json")
EMBED_CACHE_FILE = os.path.join(_BASE, "embed_cache_v12.pkl")

st.set_page_config(page_title="RAG 챗봇 v12", page_icon="📚", layout="wide")


# =====================================================================
# Tracer — 단계별 latency + token 사용량 추적
# =====================================================================
...
# =====================================================================
# [NEW v12] Embedding Cache — 디스크 영속 캐시
# =====================================================================

class EmbeddingCache:
    """
    텍스트 → 임베딩 벡터를 md5 키로 디스크에 저장.
    동일 텍스트 재임베딩 시 API 호출 없이 반환.
    """
...
def get_embeddings(texts: list) -> np.ndarray:
    """원본 API 호출 (캐시 없음). build_index 등 배치 최초 임베딩에 사용."""
...
def get_embeddings_cached(texts: list) -> np.ndarray:
    """
    [NEW v12] 캐시 우선 임베딩.
    캐시 히트 텍스트는 API 호출 없이 반환, 미스 텍스트만 API 호출 후 저장.
    """
    if not texts:
    ...
    for text in texts:
        ...
        if key in embed_cache._cache:
            ...
            if text not in result_map:  # 같은 텍스트 중복 요청 방지
...
def compute_ndcg(ordered_items: list, score_dict: dict, k: int) -> float:
    """
    ordered_items: prefilter 후 임베딩 유사도 순으로 정렬된 아이템 리스트
    score_dict: {ordered_items에서의 인덱스: rerank 점수 (0-10)}
    k: cutoff

    의미: 임베딩 검색 순서가 리랭커 기준에 얼마나 잘 정렬되어 있는가.
    NDCG = 1.0 → 임베딩 검색이 이미 최적 순서
    NDCG < 1.0 → 리랭킹이 순서를 개선함
    """
    if not score_dict:
        return 0.0
    ...
    # DCG: 임베딩 유사도 순서 기준, 리랭크 점수를 relevance로 사용
    dcg = sum(
        score_dict.get(i, 0.0) / np.log2(i + 2)
        for i in range(n)
    )
...
def build_index(chunks: list):
    """청크 임베딩을 캐시에 저장하면서 FAISS 인덱스 구축."""
...
# =====================================================================
# [NEW v12] Hybrid 검색 — Dense + BM25 + RRF 융합
# =====================================================================
...
def retrieve_hybrid(queries: list, index, chunks: list, sources: list,
    ...
    """
    [v12] Dense(FAISS) + BM25 결과를 RRF(Reciprocal Rank Fusion)로 융합.
    - Dense: 의미 기반 검색 (임베딩 유사도)
    - BM25:  어휘 기반 검색 (BM25Okapi)
    - RRF:   두 랭킹을 1/(k+rank) 점수로 합산 후 재정렬
    use_bm25=False 또는 rank_bm25 미설치 시 Dense only.
    """
    ...
    if not use_bm25 or not BM25_AVAILABLE:
        if tracer:
            tracer.end("embedding_search",
                       ...
                       output_summary=f"Dense only: {len(dense_items)}개 수집",
                       decision="BM25 비활성화 또는 미설치 (rank_bm25 패키지 필요)")
        return dense_items
    ...
    if tracer:
        tracer.end("embedding_search",
                   ...
                   input_summary=f"쿼리 {len(queries)}개",
                   output_summary=(
                       f"Dense {len(dense_items)}개 + BM25 {len(bm25_items)}개 "
                       f"→ RRF 융합 {len(result)}개"
                   ),
                   decision=f"RRF(k={RRF_K}): 어휘+의미 통합 검색")
    return result


def prefilter_by_similarity(query: str, items: list, top_n: int = 10,
                             tracer: Tracer = None) -> list:
    """[v12] 청크 임베딩 캐시 사용 → 반복 질문 시 API 호출 없음."""
    ...
    if tracer:
        tracer.end("prefilter",
                   ...
                   output_summary=f"상위 {len(result)}개 선별 (컷오프 유사도: {cutoff})",
                   decision=f"캐시 임베딩 코사인 유사도 ≥ {cutoff} 인 {top_n}개 통과")
    return result
...
def rerank_chunks(query: str, items: list, top_k: int = 3,
    ...
    """
    [v12] (top_k 결과, 전체 점수 dict) 튜플 반환.
    전체 점수 dict는 NDCG@k 계산에 사용됨.
    """
    ...
    return result, all_scores  # all_scores → NDCG 계산용
...
with st.sidebar:
    ...
    if uploaded_files:
        if st.button("문서 처리하기", use_container_width=True, type="primary"):
            ...
            st.success(f"완료! {len(all_chunks)}개 청크 (임베딩 캐시: {embed_cache.size()}개)")
    ...
    # BM25 Hybrid 토글
    if BM25_AVAILABLE:
        use_bm25 = st.toggle("Hybrid 검색 (Dense + BM25)", value=True)
    else:
        st.warning("rank_bm25 미설치 → Dense only\n`pip install rank-bm25`")
        use_bm25 = False
    ...
    st.markdown("---")
    # 캐시 상태
    st.caption(f"💾 임베딩 캐시: {embed_cache.size()}개 저장")
    hit_total = embed_cache.hits + embed_cache.misses
    if hit_total > 0:
        st.caption(f"   히트율: {embed_cache.hits}/{hit_total} ({embed_cache.hits*100//hit_total}%)")
    ...
    if st.button("임베딩 캐시 초기화", use_container_width=True):
        ...
        st.rerun()
    st.caption("v12: Hybrid 검색 + 임베딩 캐시 + NDCG@k")
        ...
        with st.chat_message("assistant"):
            if st.session_state.index is None:
                ...
                with st.spinner("답변 생성 중..."):
                    ...
                    if use_reranking and len(filtered) > top_k:
                        with st.status("🔄 리랭킹...", expanded=False) as s:
                            ranked, all_rerank_scores = rerank_chunks(prompt, filtered, top_k, tracer)
                            # [NEW v12] NDCG@k: 임베딩 검색 순서 vs 리랭커 판단
                            ndcg_k = compute_ndcg(filtered, all_rerank_scores, k=top_k)
                            s.update(label=f"✅ 상위 {top_k}개 | NDCG@{top_k}: {ndcg_k:.3f}", state="complete")
                    else:
                    ...
                    if auto_evaluate:
                        with st.status("🧪 평가 중...", expanded=False) as s:
                            ...
                            save_log(build_log_entry(
                                ...
                                evaluation, hall_cause, tracer, mode, ndcg=ndcg_k
                            ))
                            ...
                            s.update(
                                label=(
                                    ...
                                    f"환각 {hall}"
                                    + (f" · NDCG@{top_k} {ndcg_k:.3f}" if ndcg_k is not None else "")
                                ),
                                state="complete"
                            )
                ...
                if evaluation:
                    ...
                    cols[5].metric(f"📊 NDCG@{top_k}", f"{ndcg_k:.3f}" if ndcg_k is not None else "-")
                    if hall_cause:
                        st.error(f"⚠️ 환각 유형: **{hall_cause.get('환각_유형','')}** | 원인: {hall_cause.get('발생_원인','')} | 개선: {hall_cause.get('개선_제안','')}")

                # expander (쿼리, step1, step2)
                if use_query_rewrite:
...
with tab_trace:
    st.title("🔬 트레이싱 대시보드")
    st.caption("단계별 latency · 토큰 사용량 · 병목 구간 · 캐시 히트율 · NDCG@k")
    ...
    if not logs:
        ...
        ndcg_vals  = [l.get("ndcg_at_k") for l in logs if l.get("ndcg_at_k") is not None]
        avg_ndcg   = round(sum(ndcg_vals) / len(ndcg_vals), 3) if ndcg_vals else None

        # 캐시 히트율 (마지막 로그 기준)
        last = logs[-1]
        hits   = last.get("embed_cache_hits", 0)
        misses = last.get("embed_cache_misses", 0)
        hit_rate = f"{hits*100//(hits+misses)}%" if (hits + misses) > 0 else "-"
        ...
        c5.metric("임베딩 캐시 히트율",      hit_rate)
        c6.metric(f"평균 NDCG@k",           str(avg_ndcg) if avg_ndcg is not None else "-")
        ...
        # [NEW v12] NDCG 추이 차트
        if ndcg_vals:
            st.subheader(f"NDCG@k 추이 (검색 품질)")
            st.caption("1.0 = 임베딩 검색이 이미 최적 순서 / 낮을수록 리랭킹 기여도 높음")
            ndcg_series = [l.get("ndcg_at_k") for l in logs]
            ndcg_series_filled = [v if v is not None else 0 for v in ndcg_series]
            st.line_chart(ndcg_series_filled)
        ...
        for log in reversed(logs):
            ...
            ndcg_v   = log.get("ndcg_at_k")

            label = (f"[{ts}] {q}... | ⏱ {total_ms/1000:.1f}s | "
                     f"🔤 {tok.get('total',0):,} tokens | 병목: {bn}"
                     + (f" | NDCG {ndcg_v:.3f}" if ndcg_v is not None else ""))
            with st.expander(label):
                ...
                if tok_data:
                    ...
                    cc = st.columns(3)
                    cc[0].metric("Prompt 토큰",     tok.get("prompt", 0))
                    cc[1].metric("Completion 토큰",  tok.get("completion", 0))
                    cc[2].metric("총 토큰",           tok.get("total", 0))

                st.markdown("---")
                st.markdown("**🔗 API 호출 흐름**")
                flow_rows = [{
                    "단계": s["name"], "소요(ms)": s["duration_ms"],
                    "토큰": s["tokens"]["total"],
                    "입력": s.get("input_summary", "")[:60],
                    "출력": s.get("output_summary", "")[:60],
                } for s in spans]
                if flow_rows:
                    import pandas as pd
...
with tab_agent:
    ...
    st.caption("AI 의사결정 추적 · 환각 원인 심층 분석 · NDCG@k 검색 품질 분석")
    ...
    if not logs:
        ...
        c5.metric("평균 NDCG@k",      str(avg_ndcg) if avg_ndcg is not None else "-")
        ...
        # [NEW v12] NDCG 추이
        if ndcg_vals:
            st.subheader("📊 NDCG@k 추이 — 임베딩 검색 품질")
            st.caption("리랭킹이 없어도 임베딩 검색이 이미 좋은 순서라면 NDCG ≈ 1.0")
            ndcg_series = [l.get("ndcg_at_k") for l in logs]
            st.line_chart([v if v is not None else 0 for v in ndcg_series])
        ...
        for log in reversed(logs):
            ...
            label = (f"[{ts}] {q}... | 정확도 {ev.get('정확도','-')}/5 · "
                     f"관련성 {ev.get('관련성','-')}/5 · {hi} 환각 {hall}"
                     + (f" · NDCG {ndcg_v:.3f}" if ndcg_v is not None else ""))
            with st.expander(label):
        ...
        writer.writerow(["timestamp", "trace_id", "question", "정확도", "관련성",
                         "환각여부", "환각유형", "발생원인", "개선제안",
                         "total_latency_ms", "total_tokens", "bottleneck",
                         "ndcg_at_k", "mode"])
        for log in logs:
            ev = log.get("evaluation", {})
            ...
            writer.writerow(
                ...
                log.get("bottleneck",""),     log.get("ndcg_at_k",""),  log.get("mode","")
            ])
        st.download_button(
            "⬇️ 분석 로그 CSV 다운로드",
