...
LOG_FILE             = os.path.join(_BASE, "rag_eval_logs_v20.json")
EMBED_CACHE_FILE     = os.path.join(_BASE, "embed_cache_v20.pkl")
ANSWER_CACHE_FILE    = os.path.join(_BASE, "answer_cache_v20.json")
FAILURE_DATASET_FILE = os.path.join(_BASE, "failure_dataset_v20.json")  # [NEW v20]

ANSWER_CACHE_TTL_SEC     = 1800   # 30분
QUERY_CACHE_TTL_SEC      = 3600   # 1시간
FAILURE_THRESHOLD_ACCURACY = 3    # [NEW v20] accuracy <= 3 → failure

st.set_page_config(page_title="RAG 챗봇 v20", page_icon="📚", layout="wide")
...
# =====================================================================
# [v19] 캐시 전략 — QueryResultCache / AnswerCache
# =====================================================================
...
# =====================================================================
# [NEW v20] 실패 데이터셋 — FailureDataset
# =====================================================================

class FailureDataset:
    """
    [v20] 실패 케이스(낮은 정확도 / 환각 / 검색 실패) 별도 저장.
    로그 → 데이터셋화 → fine-tune / 청크 개선 루프의 입력 데이터.
    """
    def __init__(self, path: str):
        self.path  = path
    ...
    def export_finetune_jsonl(self) -> bytes:
        """OpenAI fine-tuning 형식 JSONL — 실패 케이스를 학습 데이터 후보로 내보내기."""
        lines = []
        for entry in self._data:
            chunks_text = "\n\n".join(
                [f"[출처 {i+1}]\n{c}" for i, c in enumerate(entry.get("chunks", []))]
            )
            hint = entry.get("improvement_hint") or ""
            assistant_content = hint if hint else entry.get("answer", "")
            msg = {
                "messages": [
                    {"role": "system",    "content": "문서 기반 어시스턴트. **📌 요약** / **📖 근거** ([출처 N] 인용) / **✅ 결론** 구조. 한국어."},
...
def classify_failure_types(evaluation: dict, quality_report: dict,
    ...
    if acc <= FAILURE_THRESHOLD_ACCURACY:
        types.append("low_accuracy")
    if rel <= FAILURE_THRESHOLD_ACCURACY:
        types.append("low_relevance")
    if hall in ("부분적", "있음"):
        types.append("hallucination")
    if miss and miss != "없음":
        types.append("incomplete_answer")
    if sqr and sqr.get("quality_label") in ("poor", "fair"):
        types.append("retrieval_failure")
    return types


def generate_improvement_hint(question: str, chunks: list, answer: str,
                               evaluation: dict, failure_types: list,
                               tracer=None) -> str:
    """
    [v20] 실패 케이스 자동 분석 → 청크/쿼리/답변 개선 방향 제시.
    LLM 1회 추가 호출.
    """
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": (
                ...
                f"관련성: {evaluation.get('관련성','-')}/5\n"
                f"환각여부: {evaluation.get('환각여부','-')}\n"
                f"누락_정보: {evaluation.get('누락_정보','-')}\n"
                f"개선_제안: {evaluation.get('개선_제안','-')}"
            )}
        ]
    )
    ...
    if tracer:
        tracer.end("improvement_hint",
                   ...
                   input_summary=f"실패 유형: {fail_str}",
                   output_summary=f"개선 힌트 {len(result)}자",
                   decision="실패 케이스 자동 분석")
    return result


def build_failure_entry(question: str, answer: str,
                         chunks: list, sources: list,
                         evaluation: dict, quality_report: dict,
                         failure_types: list,
                         improvement_hint: str = None,
                         ndcg: float = None, sqr: dict = None,
                         mode: str = "", mv_retrieval: bool = False) -> dict:
    return {
        ...
        "retrieval_info": {
            "ndcg":          ndcg,
            "quality_label": (sqr or {}).get("quality_label"),
            "mode":          mode,
...
failure_dataset    = FailureDataset(FAILURE_DATASET_FILE)  # [NEW v20]
...
# =====================================================================
# [v19] Context Compression — 핵심 문장 추출
# =====================================================================
...
def run_rag_pipeline(question: str, eff: dict,
    ...
    """
    [v20] RAG 파이프라인.
    신규: 평가 후 실패 유형 자동 분류 → FailureDataset 저장 → 개선 힌트 생성(선택)
    """
    tracer = Tracer()
    ...
    if auto_save_failure:
        ...
        if failure_types:
            hint = None
            if gen_improvement_hint and gen_chunks:
                hint = generate_improvement_hint(
                    question, gen_chunks, answer, evaluation, failure_types, tracer
                )
    ...
    return {
        ...
        "failure_types": failure_types, "failure_saved": failure_saved,  # [NEW v20]
    }
...
def build_log_entry(question, queries, ranked_items, answer,
                    ...
                    failure_types=None, failure_saved=False):  # [NEW v20]
...
# 탭 레이아웃
# =====================================================================
tab_chat, tab_trace, tab_agent, tab_ablation, tab_search, tab_failure = st.tabs(
    ["💬 챗봇", "🔬 트레이싱", "🧠 에이전트 분석", "🧬 Ablation Study", "🔍 검색 품질 분석", "🚨 실패 데이터셋"]
)
...
with st.sidebar:
    ...
    use_multi_vector = st.toggle("🔢 Multi-Vector 인덱싱", value=True,
                                  help="청크·문장·키워드 3종 벡터 동시 구축 → 검색 정확도 향상")
    ...
    st.caption(f"  답변 캐시: {answer_cache.valid_size()}개 유효")
    ...
    st.markdown("---")
    # [NEW v20] 실패 데이터셋 설정
    st.caption("🚨 실패 데이터셋 설정")
    enable_failure_save   = st.toggle("실패 케이스 자동 저장", value=True,
                                       help=f"정확도 ≤ {FAILURE_THRESHOLD_ACCURACY} 또는 환각/검색 실패 시 자동 저장")
    enable_hint_gen       = st.toggle("개선 힌트 자동 생성 (+1 LLM 호출)", value=False,
                                       help="실패 시 청크/쿼리/답변 개선 방향을 LLM으로 자동 분석",
                                       disabled=not enable_failure_save)
    st.caption(f"  저장된 실패 케이스: {failure_dataset.size()}건")
    ...
    if st.button("캐시 전체 초기화", use_container_width=True):
        ...
        answer_cache.clear()
        st.rerun()
    if st.button("실패 데이터셋 초기화", use_container_width=True):  # [NEW v20]
        failure_dataset.clear()
        st.rerun()
    st.caption("v20: 실패 케이스 저장 + 데이터셋화 + Auto Improvement")
        ...
        with st.chat_message("assistant"):
            if st.session_state.index is None:
                ...
                with st.status("🤔 답변 생성 중 (시도 1)...", expanded=False) as s:
                    ...
                    fail_icon    = " 🚨" if cur_result.get("failure_saved") else ""  # [NEW v20]
                    status_icon  = "✅" if not fb_needed else "⚠️"
                    s.update(
                        label=(
                            f"{status_icon} 시도 1 완료{cache_icon}{comp_icon}{refine_icon}{fail_icon} | "
                ...
                # ── 답변 표시 ───────────────────────────────────
                if cache_hit == "answer":
                    st.success(f"⚡ 답변 캐시 히트 — TTL {ANSWER_CACHE_TTL_SEC//60}분 이내 캐시 재사용")
                ...
                if failure_saved and failure_types:
                    ...
                    st.warning(f"🚨 실패 케이스 저장됨 — {labels} | 🚨 실패 데이터셋 탭에서 확인/내보내기 가능")
...
with tab_trace:
    ...
    if not logs:
        ...
        fail_count  = sum(1 for l in logs if l.get("failure_saved"))  # [NEW v20]
        ...
        c5.metric("⚡ 캐시 히트",    f"{cache_hits}회")
        c6.metric("🚨 실패 저장",    f"{fail_count}건")  # [NEW v20]
        ...
        for log in reversed(logs):
            ...
            fl_tag   = " | 🚨" if log.get("failure_saved") else ""  # [NEW v20]

            with st.expander(
                f"[{ts}] {q}... | ⏱ {total_ms/1000:.1f}s | 🔤 {tok.get('total',0):,} | {intent}"
                + (f" | NDCG {nd:.3f}" if nd else "") + fb_tag + sr_tag + ch_tag + fl_tag
            ):
                for span in spans:
...
with tab_agent:
    ...
    if not logs:
        ...
        fail_rate      = round(len(fail_logs)/len(logs)*100) if logs else 0
        all_fail_types = []
        for l in fail_logs:
            all_fail_types.extend(l.get("failure_types", []))
        type_counts = Counter(all_fail_types)

        st.subheader(f"🚨 실패 케이스 분석 ({len(fail_logs)}건 / {fail_rate}%)")
        fa1, fa2, fa3, fa4 = st.columns(4)
        fa1.metric("실패 케이스",  f"{len(fail_logs)}건")
        fa2.metric("실패율",       f"{fail_rate}%")
        fa3.metric("낮은 정확도",  f"{type_counts.get('low_accuracy', 0)}건")
        fa4.metric("환각 발생",    f"{type_counts.get('hallucination', 0)}건")
        ...
        if type_counts:
            ...
            labeled = {type_labels_map.get(k, k): v for k, v in type_counts.items()}
        ...
        cc4.metric("예상 절감 토큰",  f"{len(ans_hit_logs) * avg_tokens:,}")
        ...
        for log in reversed(logs[:10]):
            ...
            with st.expander(
                f"[{ts}] {q}... | {ev.get('정확도','-')}/5 · {hi} {hall} | 등급 {grade}({score}) | {intent}{fb_tag}{ch_tag}{fl_tag}"
                ...
                with col_a:
                    ...
                    if log.get("failure_saved"):
                        ftypes = log.get("failure_types", [])
                        st.warning(f"🚨 실패 저장: {', '.join(ftypes)}")
...
with tab_search:
    ...
    if not sq_logs:
        ...
        rows = [{"시간":         l.get("timestamp",""),
                 ...
                 "Multi-Vector": "🔢" if l.get("mv_retrieval") else "-",
                 "캐시 히트":    f"⚡{l['cache_hit']}" if l.get("cache_hit") else "-",
                 "Fallback":     "🔄" if l.get("fallback_triggered") else "-",
                 "Self-Refine":  "✏️" if l.get("self_refinement") else "-",
                 "실패 저장":    "🚨" if l.get("failure_saved") else "-"}  # [NEW v20]
                for l in sq_logs]
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)


# =====================================================================
# TAB 6 — 실패 데이터셋  [NEW v20]
# =====================================================================
with tab_failure:
    st.title("🚨 실패 데이터셋")
    st.caption("낮은 정확도 / 환각 / 검색 실패 케이스를 별도 저장 → fine-tune 데이터셋 / 청크 개선 입력으로 활용")

    entries = failure_dataset.get_all()

    if not entries:
        st.info("아직 저장된 실패 케이스가 없습니다. 챗봇 탭에서 질문하면 실패 기준 충족 시 자동 저장됩니다.")
    else:
        # ── 요약 대시보드 ──────────────────────────────────────────
        all_types = []
        for e in entries:
            all_types.extend(e.get("failure_types", []))
        ...
        d1.metric("총 실패 케이스",   len(entries))
        d2.metric("낮은 정확도",      type_counts.get("low_accuracy", 0))
        d3.metric("환각 발생",        type_counts.get("hallucination", 0))
        ...
        with col_f1:
            filter_type = st.selectbox(
                "실패 유형 필터",
                options=["전체"] + list(type_labels_map.keys()),
                format_func=lambda x: "전체" if x == "전체" else type_labels_map.get(x, x)
            )
        with col_f2:
            sort_by = st.selectbox("정렬 기준", ["최신순", "정확도 낮은순", "오래된순"])

        filtered_entries = entries if filter_type == "전체" else [
            e for e in entries if filter_type in e.get("failure_types", [])
        ]

        if sort_by == "최신순":
            filtered_entries = list(reversed(filtered_entries))
        elif sort_by == "정확도 낮은순":
            filtered_entries = sorted(filtered_entries,
                                       key=lambda e: (e.get("evaluation") or {}).get("정확도", 5))
        # 오래된순은 기본 순서 유지

        st.caption(f"표시 중: {len(filtered_entries)}건 / 전체 {len(entries)}건")
        ...
        with exp_col1:
            st.download_button(
                ...
                help="OpenAI fine-tuning API에 직접 사용 가능한 JSONL 형식"
            )
        with exp_col2:
            st.download_button(
                "⬇️ 문제 분석 JSON 내보내기",
                ...
                use_container_width=True,
                help="질문·실패유형·평가·개선힌트 요약 JSON"
            )

        # ── 케이스별 상세 ─────────────────────────────────────────
        st.markdown("---")
        st.subheader("실패 케이스 상세")
        for entry in filtered_entries:
            ...
            hall  = ev.get("환각여부", "-")
            hi    = "🟢" if hall == "없음" else ("🟡" if hall == "부분적" else "🔴")
            ...
            with st.expander(
                f"[{ts}] {q}... | 정확도 {acc}/5 · {hi} {hall} · 등급 {grade} | 🚨 {type_str}"
            ):
                tabs_inner = st.tabs(["📋 개요", "💬 답변", "📄 청크", "💡 개선 힌트"])

                with tabs_inner[0]:
                    col_i1, col_i2 = st.columns(2)
                    with col_i1:
                        st.markdown(f"**질문**: {entry.get('question','')}")
                        st.markdown(f"**실패 유형**: {type_str}")
                        if ev:
                            st.markdown(f"**정확도**: {ev.get('정확도','-')}/5")
                            st.markdown(f"**관련성**: {ev.get('관련성','-')}/5")
                            st.markdown(f"**환각여부**: {ev.get('환각여부','-')}")
                            if ev.get("누락_정보") and ev["누락_정보"] != "없음":
                                st.markdown(f"**누락 정보**: {ev['누락_정보']}")
                            if ev.get("개선_제안"):
                                st.info(f"💡 {ev['개선_제안']}")
                    with col_i2:
                        ri = entry.get("retrieval_info", {})
                        st.markdown(f"**검색 모드**: {ri.get('mode','-')}")
                        st.markdown(f"**Multi-Vector**: {'✅' if ri.get('mv_retrieval') else '❌'}")
                        if ri.get("ndcg"):
                            st.markdown(f"**Dense NDCG**: {ri['ndcg']:.4f}")
                        if ri.get("quality_label"):
                            lc = {"excellent": "🟢", "good": "🔵", "fair": "🟡", "poor": "🔴"}.get(ri["quality_label"], "⚪")
                            st.markdown(f"**검색 품질**: {lc} {ri['quality_label']}")
                        if qr:
                            st.markdown(f"**종합점수**: {qr.get('overall_score','-')}/5 (등급 {qr.get('grade','-')})")
                            if qr.get("issues"):
                                st.markdown("**감지된 문제**")
                ...
                with tabs_inner[2]:
                    ...
                    if chunks:
                        for i, (c, src) in enumerate(zip(chunks, sources or ["알 수 없음"]*len(chunks))):
                            st.caption(f"[청크 {i+1}] {src}")
                            st.text(c[:400] + ("..." if len(c) > 400 else ""))
                            if i < len(chunks) - 1:
                                st.divider()
                    else:
                        st.caption("청크 정보 없음")
                ...
                with tabs_inner[3]:
                    ...
                    if hint:
                        ...
                        st.caption("개선 힌트가 없습니다. 사이드바에서 '개선 힌트 자동 생성'을 활성화하세요.")
                        if st.button(f"지금 개선 힌트 생성", key=f"hint_{entry.get('id','')}"):
                            with st.spinner("개선 힌트 생성 중..."):
