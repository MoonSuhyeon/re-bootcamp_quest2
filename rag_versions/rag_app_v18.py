...
LOG_FILE         = os.path.join(_BASE, "rag_eval_logs_v18.json")
EMBED_CACHE_FILE = os.path.join(_BASE, "embed_cache_v18.pkl")

st.set_page_config(page_title="RAG 챗봇 v18", page_icon="📚", layout="wide")
...
DYNAMIC_RETRIEVAL_PROFILES = {
    "definition": {
        ...
        "prefilter_delta":   3,
        "top_k_delta":       0,
        "multidoc_override": False,
        "label": "정의형 → BM25 ↑, 빠른 처리",
    },
    "factual_lookup": {
        "bm25_boost":        False,
        "rerank_force":      True,
        "prefilter_delta":   3,
        "top_k_delta":       0,
        "multidoc_override": None,
        "label": "Fact형 → Rerank 강화",
    },
    "reasoning": {
        "bm25_boost":        False,
        "rerank_force":      True,
        "prefilter_delta":   5,
        "top_k_delta":       1,
        "multidoc_override": True,
        "label": "분석형 → MultiDoc ↑, Rerank 강화",
    },
    "multi_hop": {
        "bm25_boost":        False,
        "rerank_force":      True,
...
def apply_dynamic_retrieval(intent: str, eff: dict, prefilter_n: int,
    ...
    new_multidoc = use_multidoc
    if profile["multidoc_override"] is not None:
        new_multidoc = profile["multidoc_override"]
...
def compute_search_quality_report(ndcg_prefilter: float, use_bm25: bool, n_candidates: int) -> dict:
    ...
    return {
        "ndcg_prefilter": ndcg_prefilter, "reranker_gain": reranker_gain,
...
# =====================================================================
# [NEW v18] Self-Refinement — Draft → Critique → Refine
# =====================================================================

def critique_answer(question: str, context_chunks: list, draft: str,
                    tracer: Tracer = None) -> str:
    """
    [v18] Draft 답변의 문제점·누락·부정확성을 비판적으로 분석.
    구체적이고 실행 가능한 비판을 반환.
    """
...
def refine_answer(question: str, context_chunks: list, draft: str, critique: str,
                  tracer: Tracer = None) -> str:
    """
    [v18] Critique를 반영해 Draft를 개선한 최종 답변 생성.
    """
...
# =====================================================================
# 단일 파이프라인 실행 함수 (v18: use_self_refine 추가)
# =====================================================================
...
def run_rag_pipeline(question: str, eff: dict,
                     ...
                     num_rewrites: int = 3,
                     use_session_cache: bool = True,
                     use_self_refine: bool = False) -> dict:
    """
    RAG 파이프라인 전체를 한 번 실행하고 모든 결과를 dict로 반환.
    [v18] use_self_refine=True 시 Draft→Critique→Refine 루프 실행.
    """
    tracer = Tracer()
    ...
    # 5. Answer generation
    if use_multidoc and final_chunks:
        ...
        answer    = generate_answer_simple(question, ranked, tracer)
        summaries, analysis = [], ""
        mode = "simple"

    # [NEW v18] 6. Self-Refinement: Draft → Critique → Refine
    draft_answer = answer
    critique     = None
    if use_self_refine and final_chunks:
        critique = critique_answer(question, final_chunks, draft_answer, tracer)
        answer   = refine_answer(question, final_chunks, draft_answer, critique, tracer)
    ...
    hall_cause = None
    if hall != "없음":
        hall_cause = analyze_hallucination_cause(question, final_chunks, answer, hall, tracer)
...
def run_single_config(question: str, config: dict,
    ...
    try:
        ...
        return {
            ...
            "accuracy":       r["evaluation"].get("정확도", 0),
            "relevance":      r["evaluation"].get("관련성", 0),
            "hallucination":  r["evaluation"].get("환각여부", "-"),
            "confidence":     r["evaluation"].get("신뢰도", "-"),
            "overall_score":  qr["overall_score"],
            "grade":          qr["grade"],
            "ndcg_prefilter": r["ndcg_k"],
            "reranker_gain":  sqr.get("reranker_gain"),
            "search_quality": sqr.get("quality_label"),
            "latency_ms":     r["tracer"].total_latency_ms(),
            "total_tokens":   r["tracer"].total_tokens()["total"],
            "answer":         r["answer"],
            "error":          None,
        }
    except Exception as e:
...
def build_log_entry(question, queries, ranked_items, answer,
                    ...
                    fallback_triggered=False, fallback_attempts=0,
...
with st.sidebar:
    ...
    st.caption(f"  전략: top_k↑, prefilter_n↑, 모든 기능 강제 ON")

    st.markdown("---")
    # [NEW v18] Self-Refinement 설정
    st.caption("✏️ Self-Refinement 설정")
    enable_self_refine = st.toggle("Self-Refinement (Draft→Critique→Refine)", value=True,
                                   help="초안 생성 후 자기 비판 → 개선 답변 생성 (+2 LLM 호출)")
    st.caption("  Draft → Critique → Refine")

    st.markdown("---")
    # [NEW v18] Dynamic Retrieval 설정
    st.caption("🎯 Dynamic Retrieval 설정")
    enable_dynamic_retrieval = st.toggle("의도별 검색 전략 자동 조정", value=True,
                                         help="질문 유형에 따라 BM25·Rerank·MultiDoc을 자동 조정 (라우팅 ON 시 활성화)",
                                         disabled=not auto_routing)
    if auto_routing and enable_dynamic_retrieval:
        st.caption("  정의형 → BM25 ↑")
        st.caption("  분석형/멀티홉 → MultiDoc ↑")
        st.caption("  Fact형 → Rerank 강화")
    ...
    if st.button("임베딩 캐시 초기화", use_container_width=True):
        embed_cache.clear()
        st.rerun()
    st.caption("v18: Dynamic Retrieval + Self-Refinement")


# =====================================================================
# TAB 1 — 챗봇 (v18: Dynamic Retrieval + Self-Refinement)
# =====================================================================
with tab_chat:
    st.title("💬 문서 기반 챗봇 v18")
            ...
            if st.session_state.index is None:
                ...
                if auto_routing:
                    with st.status("🧭 쿼리 라우팅...", expanded=False) as s:
                        _route_tracer = Tracer()
                        route_decision = route_query(prompt, _route_tracer)
                        intent = route_decision.get("의도", "-")
                        eff = _apply_routing(route_decision, defaults)

                        # [NEW v18] Dynamic Retrieval 프로필 적용
                        ...
                        profile_info = f" | 프로필: {dynamic_profile_label}" if dynamic_profile_label else ""
                        s.update(
                            label=f"✅ 의도: {intent} | top_k: {eff['top_k']}{profile_info}",
                            state="complete"
                        )
                ...
                with st.status("🤔 답변 생성 중 (시도 1)...", expanded=False) as s:
                    cur_result = run_rag_pipeline(
                        ...
                        prefilter_n=cur_prefilter,
                        use_multidoc=cur_multidoc,
                    ...
                    refine_tag  = " ✏️ Refined" if enable_self_refine and cur_result.get("critique") else ""
                    s.update(
                        label=(
                            f"{status_icon} 시도 1 완료{refine_tag} | "
                ...
                fallback_history.append({
                    ...
                    "accuracy":      ev0.get("정확도", 0),
                    "hallucination": ev0.get("환각여부", "-"),
                    "overall_score": qr0["overall_score"],
                    "grade":         qr0["grade"],
                ...
                while fb_needed and attempt_num < MAX_RETRIES:
                    ...
                    with st.status(
                        f"🔄 Fallback 재시도 {attempt_num}/{MAX_RETRIES} — {fb_reason}",
                        expanded=False
                    ) as s:
                        cur_result = run_rag_pipeline(
                            prompt, new_eff,
                        ...
                        s.update(
                            label=(
                                f"{'✅ 개선됨' if improved else '➡️ 유지'} "
                                f"재시도 {attempt_num} | "
                                f"정확도 {ev_n.get('정확도','-')}/5 · 환각 {ev_n.get('환각여부','-')} · 등급 {qr_n['grade']}"
                                + (f" → 추가 재시도 예정: {fb_reason}" if fb_needed else "")
                            ),
                            state="complete"
                        )
                    ...
                    fallback_history.append({
                        "attempt":       attempt_num,
                        "trigger":       fallback_history[-1].get("trigger") or fb_reason,
                        "eff":           cur_result["eff"],
                        "prefilter_n":   cur_result["prefilter_n"],
                        "accuracy":      ev_n.get("정확도", 0),
                        "hallucination": ev_n.get("환각여부", "-"),
                        "overall_score": qr_n["overall_score"],
                        "grade":         qr_n["grade"],
                        "tokens":        cur_result["tracer"].total_tokens()["total"],
                        "latency_ms":    cur_result["tracer"].total_latency_ms(),
                    })
                ...
                # Self-Refinement 로그용 객체
                self_ref_log = None
                if enable_self_refine and critique:
                    self_ref_log = {
                        "enabled":  True,
                        "draft":    draft_answer[:500],
                        "critique": critique,
                    }
                ...
                # [NEW v18] Self-Refinement expander
                if enable_self_refine and critique:
                ...
                if fallback_triggered_flag:
                    with st.expander(f"🔄 Fallback 실행 내역 ({attempt_num}회 재시도)"):
                        for h in fallback_history:
                            ...
                            label   = f"{'🏆 최종 채택' if is_best else '📋'} 시도 {h['attempt'] + 1}"
                            trigger = h.get("trigger") or "최초 시도"
                            eff_h   = h["eff"]
                            st.markdown(f"**{label}** — 트리거: `{trigger}`")
                            ...
                            st.caption(
                                f"top_k={eff_h['top_k']} | prefilter={h['prefilter_n']} | "
                                f"BM25={'ON' if eff_h['use_bm25'] else 'OFF'} | "
                                f"Rerank={'ON' if eff_h['use_reranking'] else 'OFF'} | "
                                f"Rewrite={'ON' if eff_h['use_query_rewrite'] else 'OFF'}"
                            )
                ...
                if sqr:
                    ...
                    with st.expander(f"🔍 검색 품질 리포트 — {label_color} {sqr['quality_label'].upper()}"):
                ...
                if quality_report:
                    ...
                    with st.expander(f"🩺 품질 리포트 — 등급 {grade_color} {grade} ({score}/5)"):
                        ...
                        if evaluation.get("불일치_항목") and evaluation["불일치_항목"] != "없음":
                            st.markdown(f"**🔍 불일치**: {evaluation['불일치_항목']}")
                        if evaluation.get("누락_정보") and evaluation["누락_정보"] != "없음":
                            st.markdown(f"**📭 누락**: {evaluation['누락_정보']}")
...
with tab_trace:
    ...
    if not logs:
        ...
        # [NEW v18]
        refine_count = sum(1 for l in logs if l.get("self_refinement"))
        ...
        c5.metric("🔄 Fallback 발생", f"{fb_count}회")
        c6.metric("✏️ Self-Refine 적용", f"{refine_count}회")  # [NEW v18]
        ...
        for log in reversed(logs):
            ...
            fb_tag   = f" | 🔄 {log.get('fallback_attempts',0)}회 재시도" if log.get("fallback_triggered") else ""
            sr_tag   = " | ✏️ Refined" if log.get("self_refinement") else ""  # [NEW v18]
...
# =====================================================================
# TAB 3 — 에이전트 분석 (v18: Self-Refinement + Dynamic Retrieval 통계)
# =====================================================================
with tab_agent:
    ...
    if not logs:
        ...
        avg_attempts = round(sum(l.get("fallback_attempts",0) for l in fb_logs)/len(fb_logs), 1) if fb_logs else 0

        # [NEW v18] Self-Refinement 통계
        sr_logs = [l for l in logs if l.get("self_refinement")]
        sr_rate = round(len(sr_logs)/len(logs)*100) if logs else 0
        ...
        if qr_logs and len(qr_logs) >= 2:
            st.subheader("📈 종합 품질 점수 추이")
            st.line_chart([l["quality_report"]["overall_score"] for l in qr_logs])

        # [NEW v18] Self-Refinement 분석
        st.markdown("---")
        st.subheader(f"✏️ Self-Refinement 분석 ({len(sr_logs)}건 / {sr_rate}%)")
        src1, src2, src3 = st.columns(3)
        src1.metric("Self-Refine 적용", f"{len(sr_logs)}건")
        src2.metric("적용 비율", f"{sr_rate}%")
        src3.metric("미적용", f"{len(logs) - len(sr_logs)}건")

        if sr_logs:
            st.caption("Self-Refinement를 거친 답변들의 품질 지표 (평가는 Refined 답변 기준)")
            sr_acc_vals = [l["evaluation"].get("정확도",0) for l in sr_logs if l.get("evaluation")]
            all_acc_vals = [l["evaluation"].get("정확도",0) for l in eval_logs]
            if sr_acc_vals and all_acc_vals:
                sr_avg  = round(sum(sr_acc_vals)/len(sr_acc_vals), 2)
                all_avg = round(sum(all_acc_vals)/len(all_acc_vals), 2)
                st.bar_chart({"Self-Refined 평균 정확도": sr_avg, "전체 평균 정확도": all_avg})

        # [NEW v18] Dynamic Retrieval 프로필 분포
        st.markdown("---")
        st.subheader("🎯 Dynamic Retrieval 프로필 분포")
        profile_counts = Counter(
            l.get("dynamic_retrieval_profile") or "프로필 없음" for l in logs
        )
        if profile_counts:
            st.bar_chart(dict(profile_counts))
        else:
            st.caption("Dynamic Retrieval 적용 데이터 없음")
        ...
        if fb_logs:
            ...
            for l in fb_logs:
                for h in l.get("fallback_history", []):
                    if h.get("trigger"):
                        trigger_reasons.append(h["trigger"])
            trigger_counts = Counter(trigger_reasons)
            ...
            fbc1.metric("Fallback 발생",      f"{len(fb_logs)}건")
            fbc2.metric("평균 재시도 횟수",   f"{avg_attempts}회")
            fbc3.metric("Fallback 후 정확도 개선",
                        f"{avg_first:.2f} → {avg_final:.2f}",
                        delta=f"+{avg_final-avg_first:.2f}" if avg_final > avg_first else f"{avg_final-avg_first:.2f}",
                        delta_color="normal")

            if trigger_counts:
                st.markdown("**트리거 사유 분포**")
                st.bar_chart(dict(trigger_counts))
        ...
        for log in reversed(logs[:10]):
            ...
            with st.expander(
                ...
                f"{hi} {hall} | 등급 {grade}({score}) | {intent}{fb_tag}{sr_tag}"
                + (f" | NDCG {nd:.3f}" if nd else "")
            ):
                ...
                with col_a:
                    ...
                    if log.get("dynamic_retrieval_profile"):
                        st.success(f"🎯 Dynamic Retrieval: {log['dynamic_retrieval_profile']}")
                    ...
                    # Fallback 내역
                    fb_hist = log.get("fallback_history", [])
                    if log.get("fallback_triggered") and fb_hist:
                        st.markdown(f"**🔄 Fallback 내역 ({log.get('fallback_attempts',0)}회 재시도)**")
                        for h in fb_hist:
                            icon = "🏆" if h["attempt"] == log.get("fallback_attempts", 0) else "📋"
                            st.caption(
                                f"{icon} 시도 {h['attempt']+1}: "
                                f"정확도 {h['accuracy']}/5 · 환각 {h['hallucination']} · "
                                f"등급 {h['grade']}({h['overall_score']}) | "
                                f"top_k={h['eff']['top_k']} prefilter={h['prefilter_n']}"
                            )
                    ...
                    if ha:
                        ...
                        cols_ha[0].markdown(f"유형: `{ha.get('환각_유형','-')}` | 심각도: `{ha.get('심각도','-')}/10`")
                ...
                with col_b:
                    ...
                    if log.get("fallback_triggered"):
                        st.metric("재시도",  f"{log.get('fallback_attempts',0)}회")
                    if log.get("self_refinement"):
                        st.metric("Self-Refine", "✅")
    ...
    if st.session_state.index is None:
        ...
        if st.session_state.ablation_results:
            ...
            if valid:
                ...
                st.markdown("**📋 분석 요약**")
                full_r = next((r for r in valid if r["config_id"] == "full"), None)
                for cid, label in [("no_rewrite","쿼리 리라이팅"), ("no_bm25","BM25"), ("no_rerank","리랭킹")]:
                    comp = next((r for r in valid if r["config_id"] == cid), None)
                    if full_r and comp:
                        diff = full_r["accuracy"] - comp["accuracy"]
                        st.markdown(f"- {label} 기여도: 정확도 +{diff:+.1f} ({'유효' if diff > 0 else '미미'})")
    ...
    if not sq_logs:
        ...
        if poor_logs:
            ...
            for l in sorted(poor_logs, key=lambda x: x["ndcg_at_k"]):
                ...
                with st.expander(f"{label_color} NDCG {nd:.4f} | [{l.get('timestamp','')}] {l.get('question','')[:60]}..."):
                    c_a, c_b = st.columns([3, 1])
                    with c_a:
                        st.markdown(f"**질문**: {l.get('question','')}")
                    ...
                    with c_b:
                        ...
                        st.metric("BM25",          "✅" if sqr.get("use_bm25") else "❌")
        ...
        rows = [{"시간":        l.get("timestamp",""),
                 ...
                 "Self-Refine": "✏️" if l.get("self_refinement") else "-"}  # [NEW v18]
                for l in sq_logs]
