...
LOG_FILE         = os.path.join(_BASE, "rag_eval_logs_v14.json")
EMBED_CACHE_FILE = os.path.join(_BASE, "embed_cache_v14.pkl")

st.set_page_config(page_title="RAG 챗봇 v14", page_icon="📚", layout="wide")
...
def run_single_config(question: str, config: dict,
    ...
    """
    [v14] 단일 ablation config로 파이프라인 전체 실행 후 결과 반환.
    임베딩 캐시 덕분에 반복 실행 시 embedding API 비용 없음.
    """
    tracer = Tracer()
    t_start = time.time()
    ...
    try:
        ...
        return {
            ...
            "bm25":         config["bm25"],
            "rerank":       config["rerank"],
            "accuracy": 0, "relevance": 0, "hallucination": "-",
            "latency_ms": int((time.time() - t_start) * 1000),
            "total_tokens": 0, "ndcg": None,
            "answer": "", "error": str(e),
        }
...
with st.sidebar:
    ...
    if st.button("임베딩 캐시 초기화", use_container_width=True):
        embed_cache.clear()
        st.rerun()
    st.caption("v14: Ablation Study 자동 실험")
...
with tab_agent:
    ...
    if not logs:
        ...
        st.markdown("---")
        for log in reversed(logs[:10]):  # 최신 10건만 표시
            ...
            with st.expander(f"[{ts}] {q}... | {ev.get('정확도','-')}/5 · {ev.get('관련성','-')}/5 · {hi} {hall} | {intent}" + (f" | NDCG {nd:.3f}" if nd else "")):
...
with tab_ablation:
    ...
    if st.session_state.index is None:
        ...
        est_calls = n_configs * 5  # 평균 5 LLM 호출/config
        st.info(f"선택된 config {n_configs}개 × 약 5 LLM 호출 = **약 {est_calls}회 API 호출** 예상.")
        ...
        if st.session_state.ablation_results:
            ...
            if valid:
                ...
                col1.metric("🏆 최고 정확도",  f"{best_acc['config_name'].split()[-1]}",
                            delta=f"정확도 {best_acc['accuracy']}/5")
                col2.metric("🎯 최고 관련성",  f"{best_rel['config_name'].split()[-1]}",
                ...
                col4.metric("💰 최소 토큰",    f"{cheapest['total_tokens']:,}",
                ...
                for r in results:
                    ...
                    with st.expander(f"{r['config_name']} | 정확도 {r['accuracy']}/5 · 관련성 {r['relevance']}/5 · {r['latency_ms']}ms{err_tag}"):
                ...
                abl_writer.writerow(["config_name","query_rewrite","bm25","rerank",
                                     ...
                                     "latency_ms","total_tokens","ndcg","error"])
                for r in results:
                    abl_writer.writerow([
                        r["config_name"], r["query_rewrite"], r["bm25"], r["rerank"],
                        r["accuracy"], r["relevance"], r["hallucination"],
                        r["latency_ms"], r["total_tokens"],
