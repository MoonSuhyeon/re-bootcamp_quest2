...
LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rag_eval_logs_v11.json")

st.set_page_config(page_title="RAG 챗봇 v11", page_icon="📚", layout="wide")
...
def linkify_citations(text: str) -> str:
    """
    답변 텍스트의 [출처 N]을 클릭 가능한 앵커 링크로 변환.
    <a href="#source-N"> 태그 사용 — unsafe_allow_html=True 와 함께 렌더링.
    """
    return re.sub(
        r'\[출처 (\d+)\]',
        lambda m: (
...
def evaluate_answer(question, context_chunks, answer, tracer: Tracer = None):
    """
    [v11 수정] 평가 점수 1~5 범위 강제:
    - 프롬프트에 '반드시 1~5 사이 정수, 절대로 5 초과 금지' 명시
    - 파싱 후 max(1, min(5, raw)) 클램핑으로 이중 보호
    """
...
with st.sidebar:
    ...
    if st.button("로그 초기화", use_container_width=True):
        ...
        st.success("로그 삭제 완료")
        st.rerun()
    st.caption("v11: 평가 점수 클램핑 + 출처 앵커 링크 수정")
    ...
    if prompt := st.chat_input("문서에 대해 질문하세요..."):
        ...
        with st.chat_message("assistant"):
            if st.session_state.index is None:
                ...
                # [v11 수정] [출처 N]을 클릭 가능한 앵커 링크로 변환하여 렌더링
                ...
                # [v11 수정] 출처 원문을 expander 밖에 표시 + id 앵커 태그 삽입
                # → 답변의 [출처 N] 링크가 실제 DOM 요소를 가리키도록 보장
                st.markdown("**📄 출처 원문**")
                for i, (c, src, sc) in enumerate(zip(final_chunks, final_sources, final_scores)):
                    st.markdown(
                        f'<div id="source-{i+1}"></div>',
