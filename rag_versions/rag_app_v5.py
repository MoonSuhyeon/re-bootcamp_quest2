...
def chunk_text_with_overlap(text, chunk_size=500, overlap=100):
    ...
    # 2단계: 인접 청크 끝부분을 다음 청크 앞에 overlap으로 붙이기
    if overlap <= 0 or len(raw_chunks) <= 1:
        return raw_chunks

    overlapped = [raw_chunks[0]]
    for i in range(1, len(raw_chunks)):
        prev_tail = raw_chunks[i - 1][-overlap:]  # 이전 청크의 마지막 overlap자
        overlapped.append(prev_tail + " " + raw_chunks[i])

    return overlapped


# --- [NEW] 의미 기반(Semantic) 청킹 ---
def chunk_text_semantic(text, chunk_size=500, overlap=100):
    ...
    # 3단계: 인접 문장 간 코사인 유사도 계산
    similarities = [
        float(np.dot(embeddings[i], embeddings[i + 1]))
        for i in range(len(embeddings) - 1)
    ]

    # 4단계: 유사도가 평균 - 0.5*표준편차 이하인 지점을 의미 경계로 설정
    ...
    # 5단계: 의미 경계로 나눠진 문장 그룹을 chunk_size에 맞게 묶기
    raw_chunks = []
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Powered by OpenAI + FAISS | v5: Semantic Chunking + Overlap")


# --- 메인 화면 ---
st.title("💬 문서 기반 챗봇 v5")
st.caption("Semantic Chunking + Overlap + Query Rewriting + Reranking")
