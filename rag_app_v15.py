...
LOG_FILE         = os.path.join(_BASE, "rag_eval_logs_v15.json")
EMBED_CACHE_FILE = os.path.join(_BASE, "embed_cache_v15.pkl")

st.set_page_config(page_title="RAG 챗봇 v15", page_icon="📚", layout="wide")
...
def evaluate_answer(question, context_chunks, answer, tracer: Tracer = None):
    """
    [v15] 단순 수치 평가 → 구조화된 품질 진단으로 강화.
    신규 필드: 신뢰도, 불일치_항목, 누락_정보, 개선_제안
    """
    ...
    if tracer:
        tracer.end("evaluation", tokens=_usage(response),
                   ...
                   output_summary=(
                       f"정확도 {result['정확도']}/5 · 관련성 {result['관련성']}/5 · "
                       f"환각 {result['환각여부']} · 신뢰도 {result['신뢰도']}"
                   ),
                   decision="구조화 품질 진단 (v15)")
    return result


# =====================================================================
# [NEW v15] 정교화된 환각 원인 분석
# =====================================================================

def analyze_hallucination_cause(question, context_chunks, answer, hall_type, tracer: Tracer = None):
    """
    [v15] 환각 원인 분석 강화.
    신규 필드: 심각도(1~10), 원문_인용, 수정_제안
    """
    if hall_type == "없음":
        return None
    ...
    if tracer:
        tracer.end("hallucination_analysis", tokens=_usage(response),
                   ...
                   decision="환각 감지 시에만 실행 (v15 정밀 분석)")
    return result


# =====================================================================
# [NEW v15] 품질 종합 리포트 생성
# =====================================================================

def build_quality_report(evaluation: dict, hall_cause: dict = None) -> dict:
    """
    [v15] evaluation + hall_cause를 결합해 종합 품질 리포트 생성.
    overall_score: 정확도·관련성 평균 - 환각 페널티
...
def run_single_config(question: str, config: dict,
    ...
    try:
        ...
        if config["rerank"] and len(filtered) > top_k:
            ...
            ndcg_k = compute_ndcg(filtered, all_scores, k=top_k)
        else:
        ...
        return {
            ...
            "total_tokens": 0, "ndcg": None, "answer": "", "error": str(e),
        }
...
def build_log_entry(question, queries, ranked_items, answer,
                    ...
                    ndcg=None, route_decision=None, quality_report=None):
    return {
...
with st.sidebar:
    ...
    if st.button("임베딩 캐시 초기화", use_container_width=True):
        embed_cache.clear()
        st.rerun()
    st.caption("v15: 평가 고도화 + AI 오류 원인 분석")
        ...
        with st.chat_message("assistant"):
            if st.session_state.index is None:
                ...
                with st.spinner("답변 생성 중..."):
                    ...
                    if auto_evaluate:
                        with st.status("🧪 평가 중...", expanded=False) as s:
                            ...
                            s.update(label=f"✅ 정확도 {evaluation.get('정확도','-')}/5 · 관련성 {evaluation.get('관련성','-')}/5 · 환각 {hall} · 등급 {grade}({score})", state="complete")
                ...
                if evaluation:
                    ...
                    cols[6].metric(f"📊 NDCG@{eff_top_k}", f"{ndcg_k:.3f}" if ndcg_k else "-")
...
with tab_agent:
    ...
    if not logs:
        ...
        c5.metric("평균 NDCG@k",   str(avg_ndcg) if avg_ndcg else "-")
        c6.metric("평균 종합점수",  f"{avg_overall}/5" if avg_overall else "-")  # [NEW v15]
            ...
            with st.expander(
                ...
                with col_b:
                    if ev:
                        ...
                        st.metric("신뢰도", ev.get("신뢰도", "-"))  # [NEW v15]
...
# =====================================================================
# TAB 4 — Ablation Study (v14 동일 + overall_score/grade 컬럼 추가)
# =====================================================================
with tab_ablation:
    ...
    if st.session_state.index is None:
        ...
        abl_question = st.text_input(
            ...
            help="챗봇 탭의 대화 기록과 독립적으로 실행됩니다."
        )

        config_names   = [c["name"] for c in ABLATION_CONFIGS]
        selected_names = st.multiselect("실험할 Config 선택", options=config_names,
        ...
        est_calls = n_configs * 6  # v15: 평균 6 LLM 호출/config (평가 강화)
        st.info(f"선택된 config {n_configs}개 × 약 6 LLM 호출 = **약 {est_calls}회 API 호출** 예상.")
        ...
        if st.session_state.ablation_results:
            ...
            df = pd.DataFrame([{
                ...
                "정확도 (/5)":   r["accuracy"],
                "관련성 (/5)":   r["relevance"],
                "신뢰도":        r.get("confidence", "-"),       # [NEW v15]
                "종합점수 (/5)": r.get("overall_score", "-"),    # [NEW v15]
                "등급":          r.get("grade", "-"),             # [NEW v15]
                "환각여부":      r["hallucination"],
                "응답(ms)":      r["latency_ms"],
                "토큰":          r["total_tokens"],
                "NDCG@k":        round(r["ndcg"], 3) if r.get("ndcg") else "-",
            ...
            if valid:
                ...
                best_score  = max(valid, key=lambda r: r.get("overall_score", 0))  # [NEW v15]
                ...
                col3.metric("🥇 최고 종합점수", best_score["config_name"].split()[-1],   # [NEW v15]
                ...
                col5.metric("💰 최소 토큰",     f"{cheapest['total_tokens']:,}",
                            delta=cheapest["config_name"].replace("✅","").replace("❌","").replace("🔥","").strip())
                ...
                if full_result and no_bm25:
                    ...
                    lines.append(f"- BM25 하이브리드 기여도: 정확도 +{diff:+.1f} ({'유효' if diff > 0 else '미미'})")
                ...
                with col_c1:
                    ...
                    chart_data = pd.DataFrame({
                        ...
                        "종합점수": [r.get("overall_score", 0) for r in valid],  # [NEW v15]
                    }).set_index("Config")
                ...
                with col_c2:
                    ...
                    lat_data = {
                        r["config_name"].replace("✅","").replace("❌","").replace("⚡","").replace("🔥","").strip(): r["latency_ms"]
                        for r in valid
                    }
                ...
                for r in results:
                    ...
                    with st.expander(
                        f"{r['config_name']} | 정확도 {r['accuracy']}/5 · 관련성 {r['relevance']}/5 · "
                        f"{r['latency_ms']}ms{grade_tag}{err_tag}"
                    ):
                        ...
                        with col_meta:
                            ...
                            st.metric("신뢰도",   r.get("confidence", "-"))     # [NEW v15]
                            st.metric("종합점수", f"{r.get('overall_score','-')}/5")  # [NEW v15]
                            st.metric("등급",     r.get("grade", "-"))           # [NEW v15]
                            st.metric("환각",     r["hallucination"])
                            st.metric("응답",     f"{r['latency_ms']/1000:.1f}s")
                            st.metric("토큰",     f"{r['total_tokens']:,}")
                            if r.get("ndcg"):
                                st.metric("NDCG@k", f"{r['ndcg']:.3f}")
