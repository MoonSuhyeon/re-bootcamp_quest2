...
LOG_FILE         = os.path.join(_BASE, "rag_eval_logs_v17.json")
EMBED_CACHE_FILE = os.path.join(_BASE, "embed_cache_v17.pkl")

st.set_page_config(page_title="RAG 챗봇 v17", page_icon="📚", layout="wide")
...
FALLBACK_MIN_ACC     = 3    # 이 미만이면 fallback 트리거
FALLBACK_HALL_TYPES  = ("부분적", "있음")  # 이 환각 유형이면 fallback 트리거


def should_fallback(evaluation: dict) -> tuple:
    """
    [v17] 평가 결과를 보고 재시도 여부와 이유를 반환.
...
def escalate_params(base_eff: dict, base_prefilter_n: int, attempt: int) -> tuple:
    """
    [v17] 재시도 시 파라미터를 강화해 반환.
    attempt 1: top_k+1, prefilter_n+5, 모든 기능 ON
    attempt 2: top_k+2, prefilter_n+10, 모든 기능 ON
    """
...
# =====================================================================
# [NEW v17] 단일 파이프라인 실행 함수 (Fallback 루프에서 반복 호출)
# =====================================================================
...
def run_rag_pipeline(question: str, eff: dict,
    ...
    """
    [v17] RAG 파이프라인 전체를 한 번 실행하고 모든 결과를 dict로 반환.
    Fallback 루프에서 파라미터를 바꿔가며 반복 호출할 수 있도록 분리.
    """
    tracer = Tracer()
...
# =====================================================================
# Ablation Study (v17: run_rag_pipeline 활용)
# =====================================================================
...
with st.sidebar:
    ...
    if st.button("임베딩 캐시 초기화", use_container_width=True):
        embed_cache.clear()
        st.rerun()
    st.caption("v17: Fallback 자동 재시도 시스템")


# =====================================================================
# TAB 1 — 챗봇 (v17: Fallback 루프)
# =====================================================================
with tab_chat:
    st.title("💬 문서 기반 챗봇 v17")
        ...
        with st.chat_message("assistant"):
            if st.session_state.index is None:
                ...
                with st.status("🤔 답변 생성 중 (시도 1)...", expanded=False) as s:
                    ...
                    s.update(
                        label=f"{status_icon} 시도 1 완료 | 정확도 {ev0.get('정확도','-')}/5 · 환각 {ev0.get('환각여부','-')} · 등급 {qr0['grade']}"
                ...
                while fb_needed and attempt_num < MAX_RETRIES:
                    ...
                    new_eff, new_pf_n = escalate_params(eff, prefilter_n, attempt_num)
                ...
                # [NEW v17] Fallback 히스토리 expander
                if fallback_triggered_flag:
...
with tab_trace:
    ...
    if not logs:
        ...
        fb_count = sum(1 for l in logs if l.get("fallback_triggered"))  # [NEW v17]
        ...
        c6.metric("🔄 Fallback 발생", f"{fb_count}회")  # [NEW v17]
...
# =====================================================================
# TAB 3 — 에이전트 분석 (v17: Fallback 통계 추가)
# =====================================================================
with tab_agent:
    ...
    if not logs:
        ...
        c6.metric("🔄 Fallback 율", f"{fb_rate}%")  # [NEW v17]
        ...
        for log in reversed(logs[:10]):
            ...
            with st.expander(
                ...
                f"{hi} {hall} | 등급 {grade}({score}) | {intent}{fb_tag}"
                + (f" | NDCG {nd:.3f}" if nd else "")
            ):
...
with tab_search:
    ...
    if not sq_logs:
        ...
        rows = [{"시간": l.get("timestamp",""), "질문": l.get("question","")[:50],
                 "Dense NDCG": l["ndcg_at_k"], "Reranker Gain": l.get("reranker_gain","-"),
                 ...
                 "Fallback": "🔄" if l.get("fallback_triggered") else "-"}  # [NEW v17]
                for l in sq_logs]
