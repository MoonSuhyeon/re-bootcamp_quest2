# client_app.py — v27 Streamlit 프론트엔드
...
def api_get_detail(trace_id: str) -> dict | None:
    """GET /logs/{trace_id} — 상세 trace 조회 (에러 시 None)"""
    return api_get(f"/logs/{trace_id}")
...
def render_login():
    st.title("🔐 RAG App v27")
    st.caption("Corrective RAG (CRAG) · Streaming SSE · RAGAS Evaluation")
...
def render_sidebar() -> dict:
    ...
    with st.sidebar:
        ...
        # ── [NEW v27] CRAG ───────────────────────────────────────────
        st.subheader("[NEW v27] Corrective RAG")
        use_crag = st.checkbox(
            "CRAG (Corrective RAG)",
            value=False,
            help="검색 결과를 0~10으로 채점 → Ambiguous/Incorrect 이면 웹 검색 보완",
        )
        if use_crag:
            st.caption("📋 Grade → Branch → WebSearch 파이프라인 활성화")
...
def render_chat_tab(cfg: dict):
    ...
    # [NEW v26] 스트리밍 토글
    use_stream = st.toggle("⚡ 스트리밍 응답", value=False, help="실시간 토큰 출력 (RAGAS 로그 자동 저장)")
        ...
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant" and msg.get("meta"):
                ...
                if meta.get("trace_id"):
                    with st.expander("🔍 상세 분석 (NDCG · Spans · Fallback · 압축)"):
                        detail = api_get_detail(meta["trace_id"])
                        render_trace_detail(detail)
    ...
    if question:
        ...
        with st.chat_message("assistant"):
            if use_stream:
                # ── 스트리밍 모드 (v27: CRAG status 이벤트 포함) ─────
                ...
                try:
                    ...
                    with requests.post(
                        ...
                        json={
                            ...
                            "prefilter_n":         cfg.get("prefilter_n", 10),
                            "use_rewrite":         cfg.get("use_rewrite", True),
                            "use_reranking":       cfg.get("use_reranking", True),
                            "use_parallel_search": cfg.get("use_parallel_search", True),
                ...
                latency = done_meta.get("latency_ms", 0)
                st.caption(f"⚡ 스트리밍 완료 · {latency:.0f}ms · RAGAS 로그 저장됨")

                # [NEW v27] CRAG 채점 결과 + 웹 검색 쿼리 표시
                crag_grade = done_meta.get("crag_grade")
                if crag_grade:
                    label      = crag_grade.get("label", "")
                    score      = crag_grade.get("score", 0)
                    reason     = crag_grade.get("reason", "")
                    color      = {"Correct": "🟢", "Ambiguous": "🟡", "Incorrect": "🔴"}.get(label, "⚪")
                    web_q      = done_meta.get("web_query_used", "")
                    grade_line = f"{color} CRAG 등급: **{label}** (점수 {score:.1f}/10) — {reason}"
                    if web_q:
                        grade_line += f'\n\n🔍 웹 검색 쿼리: `{web_q}`'
                ...
                if done_meta.get("sources"):
                    with st.expander("📄 출처"):
                        for s in set(done_meta["sources"]):
                            st.caption(s)

                st.session_state["chat_history"].append({
                    "role": "assistant",
                    "content": full_text,
                    "meta": {"latency_ms": latency, "mode_used": "stream",
                             "sources": done_meta.get("sources", []),
                             "crag_grade": crag_grade,
                             "web_query_used": done_meta.get("web_query_used", "")},
                })
            else:
                # ── 기존 일반 모드 ────────────────────────────────────
                ...
                if result:
                    ...
                    meta = {k: result.get(k) for k in
                            ["latency_ms", "mode_used", "evaluation", "quality_report",
                             "failure_types", "failure_saved", "tool_calls", "sources",
                             "async_used", "trace_id"]}
...
# =====================================================================
# 상세 trace 렌더링 (채팅 탭 + 로그 탭 공용)
# =====================================================================

def render_trace_detail(detail: dict):
    """GET /logs/{trace_id} 응답을 탭으로 시각화."""
    if not detail:
        st.warning("상세 데이터 없음")
        return

    inner = st.tabs(["📊 검색 품질", "🔄 쿼리 / 라우팅", "⏱ Trace Spans", "🔁 Fallback", "🗜 압축 통계"])

    # ── 검색 품질 ─────────────────────────────────────────────────────
    with inner[0]:
        sqr = detail.get("search_quality_report") or {}
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("NDCG@k",       round(detail.get("ndcg_at_k") or 0, 4))
        col2.metric("Reranker Gain", round(sqr.get("reranker_gain") or 0, 4))
        col3.metric("검색 품질",     sqr.get("quality_label", "-"))
        col4.metric("병렬 검색(ms)", detail.get("parallel_ms") or "-")

        if sqr.get("diagnosis"):
            st.caption(f"진단: {sqr['diagnosis']}")

        if detail.get("retrieved_chunks"):
            st.subheader("검색된 청크")
            for i, chunk in enumerate(detail["retrieved_chunks"], 1):
                score = chunk.get("rerank_score")
                label = f"[{i}] {chunk.get('source', '')}  점수: {score}" if score else f"[{i}] {chunk.get('source', '')}"
    ...
    with inner[1]:
        ...
        if queries:
            st.subheader("재작성된 쿼리")
            for i, q in enumerate(queries):
                label = "원본" if i == 0 else f"재작성 {i}"
                st.write(f"**{label}:** {q}")
        ...
        if route:
            ...
            col1.write(f"**의도:** {route.get('의도', '-')}")
            col2.write(f"**설명:** {route.get('설명', '-')}")
            s = route.get("검색_전략", {})
            if s:
                st.json(s)

        if detail.get("dynamic_retrieval_profile"):
            st.caption(f"Dynamic Retrieval: {detail['dynamic_retrieval_profile']}")

        st.caption(f"캐시 히트: {detail.get('cache_hit') or '없음'}  |  "
                   f"MV 검색: {'✅' if detail.get('mv_retrieval') else '❌'}  |  "
                   f"엔진: {detail.get('engine', 'sync')}")
    ...
    with inner[2]:
        ...
        col1.metric("총 지연(ms)", detail.get("total_latency_ms", 0))
        col2.metric("총 토큰",     tokens.get("total", 0))
        col3.metric("병목",        detail.get("bottleneck", "-"))
        ...
        if spans:
            ...
            for s in spans:
                rows.append({
                    "단계":        s["name"],
                    "시간(ms)":    s["duration_ms"],
                    "토큰":        s["tokens"].get("total", 0),
                    "입력 요약":   s.get("input_summary", "")[:60],
                    "출력 요약":   s.get("output_summary", "")[:60],
                    "결정":        s.get("decision", "")[:60],
                })
            st.dataframe(pd.DataFrame(rows), use_container_width=True)

        hall_cause = detail.get("hallucination_analysis")
        if hall_cause:
            st.subheader("🔴 환각 원인 분석")
            c1, c2 = st.columns(2)
            c1.write(f"**유형:** {hall_cause.get('환각_유형', '-')}")
            c2.write(f"**심각도:** {hall_cause.get('심각도', '-')}/10")
            st.write(f"**주장:** {hall_cause.get('환각_주장', '-')}")
            st.write(f"**원인:** {hall_cause.get('발생_원인', '-')}")
            st.write(f"**수정 제안:** {hall_cause.get('수정_제안', '-')}")

    # ── Fallback ──────────────────────────────────────────────────────
    with inner[3]:
        triggered = detail.get("fallback_triggered", False)
        attempts  = detail.get("fallback_attempts", 0)
        history   = detail.get("fallback_history", [])

        col1, col2 = st.columns(2)
        col1.metric("Fallback 발생", "✅ 예" if triggered else "❌ 아니오")
        col2.metric("재시도 횟수",    attempts)

        if history:
            rows = []
            for h in history:
                rows.append({
                    "시도":     h.get("attempt", 0) + 1,
                    "정확도":   h.get("accuracy", 0),
                    "환각":     h.get("hallucination", "-"),
                    "등급":     h.get("grade", "-"),
                    "점수":     h.get("overall_score", 0),
                    "토큰":     h.get("tokens", 0),
                })
            st.dataframe(pd.DataFrame(rows), use_container_width=True)
    ...
    # ── 압축 통계 ─────────────────────────────────────────────────────
    with inner[4]:
        comp  = detail.get("compression_stats")
        sel   = detail.get("selective_stats")
        llm_c = detail.get("compress_stats_llm")

        if comp:
            st.subheader("Phase 2 — 임베딩 압축")
            st.json(comp)
        if sel:
            st.subheader("Phase 2 — Selective Context")
            cols = st.columns(3)
            cols[0].metric("원본 문장",   sel.get("original_sents", 0))
            cols[1].metric("중복 제거 후", sel.get("after_dedup", 0))
            cols[2].metric("압축률",       f"{sel.get('ratio', 1.0):.0%}")
        if llm_c:
            st.subheader("Phase 3 — LLM 압축")
            cols = st.columns(3)
            cols[0].metric("전체 문장",   llm_c.get("total_sents", 0))
            cols[1].metric("선택된 문장", llm_c.get("kept_sents", 0))
            cols[2].metric("압축률",      f"{llm_c.get('ratio', 1.0):.0%}")

        if not comp and not sel and not llm_c:
            st.info("압축 기능이 사용되지 않았습니다")
...
def render_logs_tab():
    ...
    for log in logs:
        ...
        sqr   = log.get("search_quality_report") or {}
        rows.append({
            "시각":      log.get("timestamp", "")[:19],
            "질문":      log.get("question", "")[:50],
            "정확도":    eval_.get("정확도", "-"),
            "관련성":    eval_.get("관련성", "-"),
            "등급":      qr.get("grade", "-"),
            "NDCG":      round(log.get("ndcg_at_k") or 0, 3) or "-",
            "검색품질":  sqr.get("quality_label", "-"),
            "지연(ms)":  log.get("total_latency_ms", 0),
            "모드":      log.get("mode", "-"),
            "캐시":      "✅" if log.get("cache_hit") else "",
            "Fallback":  "🔁" if log.get("fallback_triggered") else "",
    ...
    # 정확도 추이 차트
    acc_values = [log.get("evaluation", {}).get("정확도") for log in logs
                  if log.get("evaluation", {}).get("정확도")]
    if acc_values:
    ...
    for log in logs:
        ...
        acc   = eval_.get("정확도", "-")
        grade = qr.get("grade", "-")
        fail  = " 🚨" if log.get("failure_saved") else ""
        title = f"[{log.get('timestamp','')[:16]}] {log.get('question','')[:50]}... | 정확도 {acc}/5 · {grade}{fail}"
        with st.expander(title):
            render_trace_detail(log)
...
def render_api_info_tab():
    ...
    st.subheader("[NEW v27] 파라미터")
    st.code("""\
POST /chat
{
  "question": "...",
  "use_crag": false,            # Corrective RAG (Grade → Branch → WebSearch)
...
{
  "question": "...",
  "use_crag": false,            # CRAG 스트리밍 (status 이벤트 포함)
  ...
}
""", language="json")
